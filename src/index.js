  ...
  async fetch(req) {
    ...
    if (u.pathname === "/backtest" || u.pathname === "/backtest/api") {
      try {
        ...
        const cacheKey = new Request(new URL(`/__backtest_v63_result?symbol=${symbol}&days=${days}&mode=${mode}&leverage=${leverage}`, req.url).toString(), {method:"GET"});
        ...
        const result = await runBacktestStaged({days, mode, symbol, leverage, requestUrl:req.url});
        if (result.pending) {
...
async function runBacktestStaged({days=30, mode="both", symbol="SOLUSDT", leverage=5, requestUrl}={}) {
  ...
  if(!prep.complete) return {pending:true,version:"6.3-multicoin-leverage",symbol,leverage,days,tradeMode:mode,progress:prep,message:"歷史 K 線正在分批下載並快取。頁面會自動繼續。"};
  const variants=[
  ...
  for(let wi=0;wi<windows.length;wi++){
    const w=windows[wi],raw=await loadHistoryRange(w.start-BT_WARMUP_MS,w.end,requestUrl,symbol);
  ...
  const results=variants.map(v=>aggregateVariantRuns(v,per[v.id],leverage));
  const eligible=results.filter(x=>x.trades>=5);
  return {ok:true,symbol,market:"USDT perpetual / linear",source:`Bybit ${symbol} linear · 30-day cached bundles`,mode:"V6.3 multi-coin + leverage",version:"6.3-multicoin-leverage",days,tradeMode:mode,leverage,windows:windowReports,sharedRules:{entry:"signal candle closes; enter next candle open",stop:"1.5 ATR",tp1:"1R / 30%",tp2:"2R / 30%",runner:"40% with 1.5 ATR trailing",riskPerTradePct:0.5,leverageModel:"fixed 0.5% account risk; leverage changes required margin, not R-multiple",volatilityCooldown:"1h if >3 ATR or >4%; 4h if >7%",threeLossCooldown:"6 hours",sameBarConflict:"stop first (conservative)",maxBacktestDays:365,longRangeMethod:"single continuous window with 50-day warm-up"},leaderboard:{bestByNetR:[...results].sort((a,b)=>b.netR-a.netR)[0]?.variant||null,bestByProfitFactor:[...eligible].sort((a,b)=>b.profitFactor-a.profitFactor)[0]?.variant||null,lowestDrawdown:[...results].filter(x=>x.trades>0).sort((a,b)=>a.maxDrawdownPct-b.maxDrawdownPct)[0]?.variant||null},results};
}
...
function historyChunkKey(id,requestUrl,symbol="SOLUSDT"){return new Request(new URL(`/__bt_v63_hist/${symbol}/${id}`,requestUrl).toString(),{method:"GET"});}
async function fetchHistoryBundle(a,b,symbol="SOLUSDT"){return{symbol,startTs:a,endTs:b,m15:await fetchBybitRange("15",a,b,symbol),h1:await fetchBybitRange("60",a,b,symbol),h4:await fetchBybitRange("240",a,b,symbol),createdAt:new Date().toISOString()};}
async function fetchBybitRange(interval,startTs,endTs,symbol="SOLUSDT"){let out=[],cursorEnd=endTs,pages=0;const ms=interval==="15"?15*60e3:interval==="60"?60*60e3:4*60*60e3,maxPages=Math.ceil(((endTs-startTs)/ms+2)/1000)+2;while(cursorEnd>=startTs&&pages<maxPages){const url=`${BASE}?category=linear&symbol=${symbol}&interval=${interval}&start=${Math.floor(startTs)}&end=${Math.floor(cursorEnd)}&limit=1000`,r=await fetchWithRetry(url,{label:`Bybit range ${interval}m`,retries:2}),j=await r.json(),list=j?.result?.list;if(j?.retCode!==0||!Array.isArray(list))throw new Error(`Bybit range ${interval} invalid: ${JSON.stringify(j).slice(0,160)}`);if(!list.length)break;const batch=list.map(x=>({ts:Number(x[0]),open:Number(x[1]),high:Number(x[2]),low:Number(x[3]),close:Number(x[4]),volume:Number(x[5])})).filter(x=>Number.isFinite(x.ts)&&Number.isFinite(x.open)&&Number.isFinite(x.high)&&Number.isFinite(x.low)&&Number.isFinite(x.close)&&Number.isFinite(x.volume)&&x.ts>=startTs&&x.ts<=endTs);if(!batch.length)break;out.push(...batch);const oldest=Math.min(...batch.map(x=>x.ts));if(oldest<=startTs||oldest>=cursorEnd)break;cursorEnd=oldest-1;pages++;if(list.length<1000)break;await sleep(80)}return[...new Map(out.map(x=>[x.ts,x])).values()].sort((a,b)=>a.ts-b.ts);}
async function loadHistoryRange(a,b,requestUrl,symbol="SOLUSDT"){const cache=caches.default,ids=chunkIdsForRange(a,b),m15=[],h1=[],h4=[];for(const id of ids){const h=await cache.match(historyChunkKey(id,requestUrl,symbol));if(!h)throw new Error(`歷史快取缺少 chunk ${id}，請重新整理。`);const x=await h.json();m15.push(...(x.m15||[]));h1.push(...(x.h1||[]));h4.push(...(x.h4||[]))}const clean=xs=>[...new Map(xs.filter(x=>x.ts>=a&&x.ts<=b).map(x=>[x.ts,x])).values()].sort((p,q)=>p.ts-q.ts);return{m15:clean(m15),h1:clean(h1),h4:clean(h4)};}
...
function backtestPage(r){
  ...
  const keep=(extra={})=>{const q=new URLSearchParams({symbol:r.symbol||"SOLUSDT",days:String(r.days||30),mode:r.tradeMode||"both",leverage:String(r.leverage||5),...extra});return `/backtest?${q}`};
  const cards=["C0","CL2","CS2","COMB"].map(id=>{const x=by[id]||{},lv=x.leverage||{};return `<article class="card"><div class="cardtop"><div><span class="tag">${id}</span><h2>${names[id]}</h2></div><div class="net ${Number(x.netR)>=0?'pos':'neg'}">${Number(x.netR)>=0?'+':''}${Number(x.netR||0).toFixed(2)}R</div></div><div class="desc">${esc(x.description||'')}</div><div class="stats"><div><small>交易次數</small><b>${x.trades??0}</b></div><div><small>勝率</small><b>${Number(x.winRate||0).toFixed(1)}%</b></div><div><small>Profit Factor</small><b>${Number(x.profitFactor||0).toFixed(2)}</b></div><div><small>最大回撤</small><b>${Number(x.maxDrawdownPct||0).toFixed(2)}%</b></div><div><small>最大連敗</small><b>${x.maxLossStreak??0}</b></div><div><small>100U →</small><b>${Number(x.endingEquity||100).toFixed(2)}U</b></div><div><small>🟢 多單</small><b>${x.long?.trades??0}筆 · ${Number(x.long?.netR||0)>=0?'+':''}${Number(x.long?.netR||0).toFixed(2)}R</b></div><div><small>🔴 空單</small><b>${x.short?.trades??0}筆 · ${Number(x.short?.netR||0)>=0?'+':''}${Number(x.short?.netR||0).toFixed(2)}R</b></div><div><small>${r.leverage}x 平均保證金占用</small><b>${Number(lv.avgMarginPct||0).toFixed(1)}%</b></div><div><small>${r.leverage}x 最高保證金占用</small><b>${Number(lv.maxMarginPct||0).toFixed(1)}%</b></div></div><details><summary>最近交易</summary><div class="trades">${(x.recentTrades||[]).slice().reverse().map(t=>`<div><span>${t.side==='LONG'?'🟢 多':'🔴 空'}</span><span>${Number(t.r)>=0?'+':''}${Number(t.r).toFixed(2)}R</span></div>`).join('')||'沒有交易'}</div></details></article>`}).join('');
  const lb=r.leaderboard||{},leader=id=>id?`${id} · ${names[id]||id}`:'樣本不足';
  const daysButtons=[30,90,180,365].map(d=>`<a class="${Number(r.days)===d?'on':''}" href="${keep({days:String(d)})}">${d===365?'1年':d+'天'}</a>`).join('');
  const modes=[["both","多＋空"],["long","只做多"],["short","只做空"]].map(([m,l])=>`<a class="${(r.tradeMode||'both')===m?'on':''}" href="${keep({mode:m})}">${l}</a>`).join('');
  const coins=BACKTEST_SYMBOLS.map(sym=>`<a class="${r.symbol===sym?'on':''}" href="${keep({symbol:sym})}">${sym.replace('USDT','')}</a>`).join('');
  const levs=BACKTEST_LEVERAGES.map(v=>`<a class="${Number(r.leverage)===v?'on':''}" href="${keep({leverage:String(v)})}">${v}x</a>`).join('');
  return `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${r.symbol} V6.3</title><style>*{box-sizing:border-box}body{margin:0;background:#0b0f17;color:#f5f7fb;font-family:system-ui}.w{max-width:780px;margin:auto;padding:14px 10px 36px}.hero,.leader,.card,.note{background:#111a27;border:1px solid #263348;border-radius:18px;padding:16px;margin-top:10px}.ey,.desc,.foot,small{color:#91a0b5}.hero h1{margin:7px 0}.label{font-size:12px;color:#91a0b5;margin-top:12px}.btns{display:flex;gap:7px;flex-wrap:wrap;margin-top:7px}.btns a{color:#eaf0f8;text-decoration:none;border:1px solid #2b3950;border-radius:10px;padding:8px 11px}.btns a.on{background:#f5f7fb;color:#111827}.leader div{display:flex;justify-content:space-between;padding:5px 0}.cardtop{display:flex;justify-content:space-between}.tag{border:1px solid #34435c;border-radius:8px;padding:3px 7px}.card h2{font-size:17px}.net{font-size:25px;font-weight:800}.pos{color:#58d99b}.neg{color:#ff7e87}.stats{display:grid;grid-template-columns:1fr 1fr;gap:8px}.stats div,.trades{background:#0b1420;border:1px solid #223048;border-radius:12px;padding:10px}.stats small{display:block}.trades div{display:flex;justify-content:space-between;padding:5px}.card details{margin-top:10px}.note{font-size:13px;line-height:1.6}.foot{font-size:12px;line-height:1.6;margin:14px 4px}.foot a{color:#dbe7f7}</style></head><body><main class="w"><section class="hero"><div class="ey">${r.symbol} · V6.3 MULTI-COIN + LEVERAGE</div><h1>🧪 C 策略研究回測</h1><div class="desc">${r.days} 天 · ${r.tradeMode||'both'} · ${r.leverage}x · 最長 1 年</div><div class="label">幣種</div><div class="btns">${coins}</div><div class="label">期間</div><div class="btns">${daysButtons}</div><div class="label">方向</div><div class="btns">${modes}</div><div class="label">槓桿</div><div class="btns">${levs}</div></section><section class="note">⚙️ <b>槓桿計算方式：</b>策略仍固定每筆帳戶風險 0.5%。因此 5x→10x 不會把 Net R 直接乘兩倍，而是降低建立同一風險部位所需的保證金。下方會顯示平均與最高保證金占用。</section><section class="leader"><div><span>🏆 淨 R 最高</span><b>${leader(lb.bestByNetR)}</b></div><div><span>⚡ PF 最高</span><b>${leader(lb.bestByProfitFactor)}</b></div><div><span>🛡️ 回撤最低</span><b>${leader(lb.lowestDrawdown)}</b></div></section>${cards}<div class="foot">每筆風險 0.5% · SL 1.5 ATR · TP1 1R/30% · TP2 2R/30% · Runner 40%。<br><a href="/backtest/api?symbol=${r.symbol}&days=${r.days}&mode=${r.tradeMode}&leverage=${r.leverage}">JSON API</a> · <a href="/">SOL 即時 Dashboard</a></div></main></body></html>`;
}
function backtestProgressPage(r){const p=r.progress||{},pct=p.totalChunks?Math.min(100,Math.round(p.readyChunks/p.totalChunks*100)):0;return `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="1"><title>準備回測資料</title></head><body style="margin:0;background:#0b0f17;color:#fff;font-family:system-ui;padding:22px"><div style="max-width:620px;margin:auto;background:#111a27;border:1px solid #263348;border-radius:18px;padding:20px"><div style="color:#91a0b5">${r.symbol||'SOLUSDT'} V6.3 歷史資料分批快取</div><h2>⏳ 準備 ${r.days} 天回測資料</h2><div style="font-size:32px;font-weight:800">${pct}%</div><div style="height:14px;background:#0b1420;border-radius:99px;overflow:hidden;margin:16px 0"><i style="display:block;height:100%;width:${pct}%;background:#dbe7f7"></i></div><p>${p.readyChunks||0} / ${p.totalChunks||0} 個 30 天區塊完成</p><p style="color:#91a0b5;line-height:1.6">本次新增 ${p.fetchedThisRun||0} 個，剩餘 ${p.remainingChunks||0} 個。頁面會自動繼續。</p></div></body></html>`;}

function backtestErrorPage(e){const m=String(e?.message||e||'Unknown error').replace(/[&<>]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[c]));return `<!doctype html><html lang="zh-Hant"><meta name="viewport" content="width=device-width,initial-scale=1"><body style="margin:0;background:#0b0f17;color:#fff;font-family:system-ui;padding:24px"><h2>⚠️ 回測失敗</h2><p>${m}</p><p><a style="color:#fff" href="/backtest?symbol=SOLUSDT&days=30&mode=both&leverage=5">重試 30 天</a></p></body></html>`}
