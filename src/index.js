...
export default {async fetch(req){
  ...
  try{
    ...
    const report={symbol:SYMBOL,source:"Bybit SOLUSDT Perpetual",updatedAt:new Date().toISOString(),price:data["15m"].close,overall,data};
...
function page(r){
 const rows=TFS.map(([k,l])=>{let x=r.data[k];return `<tr><td><b>${l}</b></td><td>${x.close.toFixed(2)}</td><td>${x.ema20.toFixed(2)}</td><td>${x.ema50.toFixed(2)}</td><td>${x.ema200.toFixed(2)}</td><td><b>${x.rsi14.toFixed(1)}</b><small>${x.rsiState}</small></td><td><span>${x.trend}</span></td></tr>`}).join("");
...
*{box-sizing:border-box}body{margin:0;background:#0b0f17;color:#f5f7fb;font-family:system-ui,-apple-system,sans-serif}.w{max-width:980px;margin:auto;padding:16px 12px 32px}.hero,.card{background:#121925;border:1px solid #263246;border-radius:20px}.hero{padding:20px;background:linear-gradient(145deg,#182131,#121925)}.ey{color:#8995a8;font-size:12px;letter-spacing:.08em}.top{display:flex;justify-content:space-between;align-items:end;gap:12px}h1{font-size:23px;margin:7px 0}.price{font-size:36px;font-weight:800}.badge,td span{background:#0d1420;border:1px solid #263246;border-radius:999px;padding:7px 10px;white-space:nowrap}.meta{color:#8995a8;font-size:12px;margin-top:8px}.card{margin-top:14px;overflow:hidden}.title{padding:15px;font-weight:750;border-bottom:1px solid #263246}.scroll{overflow:auto}table{width:100%;min-width:700px;border-collapse:collapse}th,td{padding:14px 11px;text-align:right;border-bottom:1px solid #263246;font-variant-numeric:tabular-nums}th{color:#8995a8;font-size:12px;background:#101722}th:first-child,td:first-child{text-align:left}small{display:block;color:#8995a8;margin-top:2px}.links{display:flex;gap:8px;margin-top:14px}.links a{color:#f5f7fb;text-decoration:none;background:#121925;border:1px solid #263246;padding:9px 12px;border-radius:12px}.foot{color:#8995a8;font-size:12px;line-height:1.6;margin-top:14px}
</style></head><body><main class="w"><section class="hero"><div class="ey">SOL TECHNICAL DASHBOARD</div><div class="top"><div><h1>SOLUSDT Perpetual</h1><div class="price">$${r.price.toFixed(2)}</div></div><div class="badge">${r.overall}</div></div><div class="meta">Bybit SOLUSDT Perpetual · 更新 <span id="t"></span></div></section><section class="card"><div class="title">多週期 EMA / RSI</div><div class="scroll"><table><thead><tr><th>週期</th><th>Close</th><th>EMA20</th><th>EMA50</th><th>EMA200</th><th>RSI14</th><th>趨勢</th></tr></thead><tbody>${rows}</tbody></table></div></section><div class="links"><a href="/api">API JSON</a><a href="/health">Health</a></div><div class="foot">EMA 使用 SMA seed 後標準 EMA 遞迴；RSI 使用 Wilder 平滑。技術指標來源為 Bybit SOLUSDT 永續，與 Binance SOLUSDC 實際成交價可能略有差異。</div></main><script>document.getElementById("t").textContent=new Date(${JSON.stringify(r.updatedAt)}).toLocaleString("zh-TW",{timeZone:"Asia/Taipei",hour12:false});setTimeout(()=>location.reload(),60000)</script></body></html>`;
}